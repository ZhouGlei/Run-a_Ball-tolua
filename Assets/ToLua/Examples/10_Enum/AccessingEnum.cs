...
public class AccessingEnum : MonoBehaviour 
{
    string script =
        @"
            ...
            function ChangeLightIntensity(light,intensity)
                
                light.intensity= light.intensity+intensity;
                print('change light intensity to '..tostring(light.intensity))
            end
        ";
    ...
    void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(0, 120, 120, 50), "ChangeIntensity"))
        {
            GameObject go = GameObject.Find("/Light");
            Light light = go.GetComponent<Light>();
            LuaFunction func2 = state.GetFunction("ChangeLightIntensity");
            func2.BeginPCall();
            func2.Push(light);
            func2.Push(1);
            func2.PCall();
